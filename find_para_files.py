...
comb = [literal_eval(line.strip()) for line in f]
...
def get_model(parameter_input):
    ...
    Returns:
        model_index(int): index of comb array corresponding to model with input parameters,
        for example model_index=4 corresponds to model4.para
    """

    parameter_list = [float(p) for p in parameter_input.split(',')]
    model_index = comb.index(tuple(parameter_list))
    return model_index